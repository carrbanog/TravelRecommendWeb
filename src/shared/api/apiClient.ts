...
export const handleApiError = (error: AxiosError | Error) => {
  // 1. 기본 메시지 세팅
  let errorMessage = "서버 요청 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.";

  // 2. 만약 커스텀 에러(프론트에서 직접 던진 에러)라면?
  if (error instanceof Error && !(axios.isAxiosError(error))) {
    errorMessage = error.message;
    console.error("[Custom Error]:", errorMessage);
  } 
  // 3. 만약 백엔드 통신 중 터진 'Axios 에러'라면?
  else if (axios.isAxiosError(error)) {
    if (error.response) {
      errorMessage = error.response.data.message || errorMessage;
    } else if (error.request) {
      errorMessage = "서버와 연결할 수 없습니다. 인터넷 연결 상태를 확인해주세요.";
    }
